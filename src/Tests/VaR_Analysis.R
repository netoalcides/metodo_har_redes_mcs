...
var_test_results <- foreach( horizons = horizons_test$pred_horizon, .combine = rbind ) %:% 
  foreach( models = models_test$model, .combine = rbind ) %dopar% {
    ...
    data_frame( model = models,
                ...
                viol_ratio = 100 * mean( ifelse( har_original_data_structure_valid$simple_return < var$VaR, 1, 0 ) ),
                asmf = 100 * (sum( ifelse( har_original_data_structure_valid$simple_return < var$VaR, (har_original_data_structure_valid$simple_return - var$VaR)^2, 0 ) ) / sum( ifelse( har_original_data_structure_valid$simple_return < var$VaR, 1, 0 ) )),
