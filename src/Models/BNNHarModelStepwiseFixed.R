...
set.seed(12345)
bnnhar_stepwise_model <- brnn( formula = log_rv252~., 
                               data = neural_data_train,
                               normalize = FALSE,
                               verbose = F,
                               neurons = best$size,
...
variables <- bnnhar_stepwise_model$coefnames

fixed_window_bnnhar_stepwise_model.average_effect <- foreach( b = 1:B, .combine = rbind ) %:% 
...
cache("fixed_window_bnnhar_stepwise_model.average_effect")
